...
var x = [
  ...
  function sorter(sortBy, opts){
	const asc = opts.asc != undefined ? opts.asc : true;
	const type = opts.type ? opts.type : 'string';
	const typeFormat = opts.typeFormat;
	return function(a, b){
		let firstElement = castType(a[sortBy], type, typeFormat);
		let secondElement = castType(b[sortBy], type, typeFormat);
...
function castType(property, type, typeFormat){
	switch(type){
		case 'string':
			return property;
		case 'number':
...
console.info(x.sort((a,b) => {
	return moment(b.release_date, 'YYYY-MM-DD').toDate().getTime() - moment(a.release_date, 'YYYY-MM-DD').toDate().getTime();
}));


const s = moment('2015-12-11', 'YYYY-MM-DD').toDate().getTime() - moment('1977-05-25', 'YYYY-MM-DD').toDate().getTime();
